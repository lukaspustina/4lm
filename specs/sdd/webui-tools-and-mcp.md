...
- **Backends shipping today** (`config/profiles/default.yaml`): `omlx` serves the full Qwen3 stack — `Qwen3-Coder-Next-4bit` (coder), `Qwen3.8-27B-4bit` (chat), `Qwen3-Embedding-8B-4bit-DWQ` (embed), `Qwen3-Reranker-0.6B-4bit-DWQ` (rerank), `Qwen3-VL-8B-4bit-DWQ` (vision). `lean.yaml` and `max-100gb.yaml` are tier variants of the same stack (`lean.yaml` keeps `Qwen3.6-35B-A3B-4bit` as its chat model); `mlx-coding.yaml` is Qwen3-Coder-only; `mlx-knowledge.yaml` is Qwen3.8-27B-only.
